...
def run_setup_wizard() -> None:
        ...
        try:
            ...
            if PKIService.verify_mkcert_available():
                ...
                if sync_result.get("status") == "error":
                    # If it's a permission error, it's fine for local dev, we just warn
                    if "Permission denied" in sync_result.get("message", ""):
                        print("ℹ️  Info: CA no pudo sincronizarse a nivel de sistema (no sudo).")
                        print("   Los certificados se usarán localmente.")
                    else:
    ...
    # 12. CREAR USUARIO ADMINISTRADOR (si no existe)
