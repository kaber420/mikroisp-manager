<script lang="ts">
    ...
    import { getRouterUsers, createRouterUser, deleteRouterUser } from '$lib/api';
    ...
    async function loadUsers() {
        ...
        try {
            ...
            error = e?.response?.data?.detail || 'Error al cargar los usuarios del router.';
        } finally {
            loading = false;
        }
    }
...
<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex gap-2">
            <button class="btn btn-sm btn-ghost" on:click={loadUsers} disabled={loading}>
                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 {loading ? 'animate-spin' : ''}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M23 4v6h-6"/><path d="M1 20v-6h6"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
                Actualizar
            </button>
            <button class="btn btn-sm btn-primary gap-2" on:click={openModal}>
    ...
    {#if error}
        <div class="alert alert-error py-2">
            ...
            <button class="btn btn-xs btn-ghost ml-auto" on:click={() => error = ''}>✕</button>
        </div>
    {/if}
                ...
                {:else}
                    {#each users as u}
                        <tr class="hover">
                            ...
                            <td class="text-xs">{u['last-logged-in-time'] || '-'}</td>
                            <td class="text-right">
                                {#if u.name !== 'admin'}
                                    <button
                                        class="btn btn-xs btn-ghost text-error"
                                        on:click={() => deleteTarget = u['.id'] || u.name}
...
{#if showModal}
    <dialog class="modal modal-open">
        <div class="modal-box max-w-sm">
            ...
            <form on:submit|preventDefault={handleCreate} class="space-y-4">
                ...
                <div class="modal-action mt-2">
                    <button type="button" class="btn btn-sm btn-ghost" on:click={() => showModal = false}>Cancelar</button>
        ...
        <div class="modal-backdrop" role="button" tabindex="-1" on:click={() => showModal = false} on:keydown={(e) => e.key === 'Escape' && (showModal = false)}></div>
    </dialog>
{/if}
...
{#if deleteTarget !== null}
    <dialog class="modal modal-open">
        <div class="modal-box max-w-sm">
            ...
            <p class="py-4 text-sm">Esta acción eliminará permanentemente el usuario
                <span class="font-mono font-bold">{deleteTarget}</span> del router.
            </p>
            <div class="modal-action">
                <button class="btn btn-sm btn-ghost" on:click={() => deleteTarget = null}>Cancelar</button>
                <button class="btn btn-sm btn-error" on:click={() => deleteTarget && handleDelete(deleteTarget)}>
                    Sí, eliminar
                </button>
            </div>
        </div>
        <!-- svelte-ignore a11y_no_static_element_interactions -->
        <div class="modal-backdrop" role="button" tabindex="-1" on:click={() => deleteTarget = null} on:keydown={(e) => e.key === 'Escape' && (deleteTarget = null)}></div>
    </dialog>
{/if}
