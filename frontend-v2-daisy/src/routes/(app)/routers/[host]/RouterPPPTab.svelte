<script lang="ts">
    ...
    async function loadServers() {
        loadingServers = true;
        try {
            const data = await getPPPoEServers(host);
            servers = Array.isArray(data) ? data : [];
        } catch (e: any) {
            notify.error(
                e?.response?.data?.detail ||
                    "Error al cargar los servidores PPPoE.",
            );
        } finally {
    ...
    async function loadProfiles() {
        ...
        try {
            ...
            notify.error(
                e?.response?.data?.detail ||
                    "Error al cargar los perfiles PPP.",
            );
        } finally {
    ...
    async function handleSaveSecret() {
        ...
        try {
            if (editingSecret) {
                ...
                if (secretForm.comment !== undefined)
                    update.comment = secretForm.comment;
                await updatePPPoESecret(host, editingSecret[".id"], update);
                notify.success(`Secreto "${secretForm.username}" actualizado.`);
            } else {
                ...
                notify.success(
                    `Secreto "${secretForm.username}" creado correctamente.`,
                );
            }
            ...
            formError =
                e?.response?.data?.detail || "Error al guardar el secreto.";
        } finally {
            savingSecret = false;
        }
    }
    ...
    async function toggleSecretStatus(s: any) {
        ...
        try {
            ...
            notify.success(
                `Secreto "${s.name}" ${shouldDisable ? "deshabilitado" : "habilitado"}.`,
            );
            await loadSecrets();
        } catch (e: any) {
            notify.error(
                e?.response?.data?.detail ||
                    "Error al cambiar el estado del secreto.",
            );
        }
    }
    ...
    async function handleKillSession(username: string) {
        try {
            ...
            notify.success(`Sesión de "${username}" desconectada.`);
            killSessionTarget = null;
            await loadActive();
        } catch (e: any) {
            notify.error(
                e?.response?.data?.detail || "Error al desconectar la sesión.",
            );
        }
    }
    ...
    async function handleSaveServer() {
        if (!serverForm.service_name.trim()) {
            serverFormError = "El nombre del servicio es requerido.";
            return;
        }
        if (!serverForm.interface) {
            serverFormError = "Debes seleccionar una interfaz.";
            return;
        }
        savingServer = true;
        serverFormError = "";
        try {
            ...
            notify.success(
                `Servidor PPPoE "${serverForm.service_name}" añadido.`,
            );
            ...
            serverFormError =
                e?.response?.data?.detail ||
                "Error al crear el servidor PPPoE.";
        } finally {
            savingServer = false;
        }
    }
    ...
    async function handleDeleteServer(serviceName: string) {
        try {
            ...
            notify.success(`Servidor "${serviceName}" eliminado.`);
            deleteServerTarget = null;
            await loadServers();
        } catch (e: any) {
            notify.error(
                e?.response?.data?.detail ||
                    "Error al eliminar el servidor PPPoE.",
            );
        }
    }
    ...
    async function handleSaveProfile() {
        if (!profileForm.plan_name.trim()) {
            profileFormError = "El nombre del plan es requerido.";
            return;
        }
        if (!profileForm.rate_limit.trim()) {
            profileFormError = "La velocidad es requerida (ej: 10M/10M).";
            return;
        }
        if (profileForm.pool_mode === "new" && !profileForm.pool_range.trim()) {
            profileFormError =
                "Debes indicar el rango del pool (ej: 192.168.20.2-192.168.20.200).";
            return;
        }
        if (
            profileForm.pool_mode === "existing" &&
            !profileForm.remote_address.trim()
        ) {
            profileFormError = "Debes seleccionar un pool existente.";
            return;
        }
        savingProfile = true;
        profileFormError = "";
        ...
        if (profileForm.pool_mode === "new")
            payload.pool_range = profileForm.pool_range;
        ...
        try {
            ...
            notify.success(
                `Perfil "${profileForm.plan_name}" creado correctamente.`,
            );
            showProfileModal = false;
            await loadProfiles();
        } catch (e: any) {
            profileFormError =
                e?.response?.data?.detail || "Error al crear el perfil.";
        } finally {
            savingProfile = false;
        }
    }
    ...
    async function handleDeleteProfile(planName: string) {
        try {
            ...
            notify.success(`Perfil "${planName}" eliminado.`);
            deleteProfileTarget = null;
            await loadProfiles();
        } catch (e: any) {
            notify.error(
                e?.response?.data?.detail || "Error al eliminar el perfil.",
            );
        }
    }
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-base-content/60">
                    Secretos, sesiones, servidores y perfiles PPPoE del router.
                </p>
            </div>
            <button
        ...
        {#if activeTab === "secrets"}
            <div class="space-y-3">
                    ...
                    <table class="table table-sm w-full">
                        <thead
                            ><tr class="bg-base-200/50">
                                <th>Usuario</th><th>Perfil</th><th>Servicio</th
                                ><th>Comentario</th><th>Estado</th><th
                                    class="text-right">Acciones</th
                                >
                            </tr></thead
                        >
                        <tbody>
                            ...
                            {:else if filteredSecrets.length === 0}
                                <tr
                                    ><td
                                        ...
                                        >{searchSecrets
                                            ? "Sin resultados."
                                            : "No hay secretos PPP."}</td
                                    ></tr
                                >
                            {:else}
                                {#each filteredSecrets as s}
                                    <tr
                                        class="hover {s.disabled === 'true' ||
                                        s.disabled === true
                                            ? 'opacity-50'
                                            : ''}"
                                    >
                                        ...
                                        >
                                        <td
                                            class="text-xs text-base-content/70 max-w-xs truncate"
                                            >{s.comment || "-"}</td
                                        >
                                        <td>
                ...
                <p class="text-xs text-base-content/40">
                    Mostrando {filteredSecrets.length} de {secrets.length} secreto(s).
                </p>
            </div>
        {/if}
            ...
            <div class="space-y-3">
                ...
                <div class="overflow-x-auto rounded-lg border border-base-300">
                    <table class="table table-sm w-full">
                        <thead
                            ><tr class="bg-base-200/50">
                                <th>Usuario</th><th>Servicio</th><th
                                    >IP Caller</th
                                ><th>IP Remota</th><th>Uptime</th><th
                                    class="text-right">Acciones</th
                                >
                            </tr></thead
                        >
                ...
                <p class="text-xs text-base-content/40">
                    {filteredSessions.length} sesión(es) activa(s).
                </p>
            </div>
        {/if}
            ...
            <div class="space-y-3">
                ...
                <div class="overflow-x-auto rounded-lg border border-base-300">
                    <table class="table table-sm w-full">
                        <thead
                            ><tr class="bg-base-200/50">
                                <th>Nombre Servicio</th><th>Interface</th><th
                                    >Perfil Defecto</th
                                ><th>1 Sesión/Host</th><th>Keepalive (s)</th><th
                                    >Estado</th
                            ...
                            {:else if servers.length === 0}
                                <tr
                                    ><td
                                        colspan="7"
                                        class="text-center py-8 text-base-content/50"
                                        >No hay servidores PPPoE configurados.</td
                                    ></tr
                                >
                            {:else}
                                {#each servers as srv}
            ...
            <div class="space-y-3">
                ...
                <div class="overflow-x-auto rounded-lg border border-base-300">
                    <table class="table table-sm w-full">
                        <thead
                            ><tr class="bg-base-200/50">
                                <th>Nombre</th><th>Rate Limit</th><th
                                    >IP Local</th
                                ><th>Pool/Remote</th><th>Cola Padre</th><th
                                    class="text-right">Acciones</th
                                >
                            </tr></thead
                        >
                            ...
                            {:else if profiles.length === 0}
                                <tr
                                    ><td
                                        colspan="6"
                                        class="text-center py-8 text-base-content/50"
                                        >No hay perfiles PPP configurados.</td
                                    ></tr
                                >
                            {:else}
                                {#each profiles as p}
                                    {@const isSystem = SYSTEM_PROFILES.includes(
                                        p.name,
                                    )}
                                    <tr
                                        class="hover {isSystem
                                            ? 'opacity-60'
                                            : ''}"
                                    >
                                        <td class="font-mono font-semibold">
                ...
                <p class="text-xs text-base-content/40">
                    {profiles.length} perfil(es) en el router.
                </p>
            </div>
        {/if}
    {/if}
    ...
    <dialog class="modal modal-open">
        <div class="modal-box max-w-md">
            <h3 class="font-bold text-lg mb-4">
                {editingSecret
                    ? `Editar Secreto: ${editingSecret.name}`
                    : "Nuevo Secreto PPP"}
            </h3>
            ...
            <form on:submit|preventDefault={handleSaveSecret} class="space-y-3">
                ...
                <label class="form-control">
                    ...
                    <input
                        ...
                        placeholder={editingSecret
                            ? "Nueva contraseña..."
                            : "Contraseña del secreto"}
                ...
                <label class="form-control">
                    ...
                    {#if profiles.length > 0}
                        ...
                        >
                            {#each profiles as p}<option value={p.name}
                                    >{p.name}</option
                                >{/each}
                        </select>
                    {:else}
    ...
    <dialog class="modal modal-open">
        <div class="modal-box max-w-sm">
            ...
            <p class="py-4 text-sm">
                Esta acción eliminará permanentemente el secreto del router.
            </p>
...
{#if killSessionTarget !== null}
    <dialog class="modal modal-open">
        <div class="modal-box max-w-sm">
            ...
            <p class="py-4 text-sm">
                El usuario <span class="font-mono font-bold"
                    >{killSessionTarget}</span
                > será desconectado inmediatamente.
            </p>
            <div class="modal-action">
                <button
                    class="btn btn-sm btn-ghost"
...
{#if showServerModal}
    <dialog class="modal modal-open">
        <div class="modal-box max-w-lg">
            ...
            {#if serverFormError}<div
                    class="alert alert-error py-2 mb-3 text-sm"
                >
                    {serverFormError}
                </div>{/if}
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <label class="form-control">
                        ...
                        <input
                            class="input input-bordered input-sm"
                            type="text"
                            bind:value={serverForm.service_name}
                            placeholder="pppoe-server"
                            required
                        />
                    </label>
                    <label class="form-control">
                        ...
                        <input
                            class="input input-bordered input-sm"
                            type="text"
                            bind:value={serverForm.interface}
                            placeholder="ether1, bridge1, etc."
                            required
                        />
                        <span class="text-xs text-base-content/50 mt-1"
                            >Nombre exacto de la interfaz del router</span
                        >
                    </label>
                </div>
                <label class="form-control">
                    ...
                    {#if profiles.length > 0}
                        <select
                            class="select select-bordered select-sm"
                            bind:value={serverForm.default_profile}
                        >
                            {#each profiles as p}<option value={p.name}
                                    >{p.name}</option
                                >{/each}
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 items-end">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input
                            ...
                            bind:checked={serverForm.one_session_per_host}
                        />
                        <div>
                            <span class="label-text font-medium"
                                >One Session per Host</span
                            >
                            <p class="text-xs text-base-content/50">
                                Evita múltiples sesiones desde el mismo cliente
                            </p>
                        </div>
                    </label>
                    <label class="form-control">
                        ...
                        <input
                            class="input input-bordered input-sm"
                            type="number"
                            bind:value={serverForm.keepalive_timeout}
                            min="1"
                            max="120"
                        />
                    </label>
...
{#if deleteServerTarget !== null}
    <dialog class="modal modal-open">
        <div class="modal-box max-w-sm">
            ...
            <p class="py-4 text-sm">
                El servidor <span class="font-mono font-bold"
                    >{deleteServerTarget}</span
                > será eliminado. Los clientes conectados perderán el acceso.
            </p>
            <div class="modal-action">
                <button
                    class="btn btn-sm btn-ghost"
    ...
    <dialog class="modal modal-open">
        <div class="modal-box max-w-lg">
            ...
            <p class="text-sm text-base-content/60 mb-4">
                Crea un perfil PPP con su pool de IPs en el router. El nombre
                del perfil se generará automáticamente como <code
                    >profile-{profileForm.plan_name
                        .toLowerCase()
                        .replace(/\s+/g, "-") || "nombre"}</code
                >.
            </p>
            {#if profileFormError}<div
                    class="alert alert-error py-2 mb-3 text-sm"
                >
                    {profileFormError}
                </div>{/if}
            ...
            >
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <label class="form-control">
                        ...
                        <input
                            class="input input-bordered input-sm"
                            type="text"
                            bind:value={profileForm.plan_name}
                            placeholder="ej. Residencial-10M"
                            required
                        />
                        <span class="text-xs text-base-content/50 mt-1"
                            >Se creará como: profile-residencial-10m</span
                        >
                    </label>
                    <label class="form-control">
                        <span class="label-text"
                            >Velocidad OK (Subida/Bajada) *</span
                        >
                        <input
                            class="input input-bordered input-sm"
                            type="text"
                            bind:value={profileForm.rate_limit}
                ...
                <label class="form-control">
                    ...
                    <input
                        class="input input-bordered input-sm"
                        type="text"
                        bind:value={profileForm.local_address}
                        placeholder="ej. 192.168.20.1"
                    />
                </label>
                <label class="form-control">
                    ...
                    <input
                        class="input input-bordered input-sm"
                        type="text"
                        bind:value={profileForm.parent_queue}
                        placeholder="none"
                    />
                    <span class="text-xs text-base-content/50 mt-1"
                        >Nombre de la cola padre o "none"</span
                    >
                </label>
                    ...
                    <div class="flex gap-4">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input
                                ...
                                bind:group={profileForm.pool_mode}
                                value="new"
                            />
                    ...
                    {#if profileForm.pool_mode === "new"}
                        <label class="form-control">
                            ...
                            <input
                                class="input input-bordered input-sm"
                                type="text"
                                bind:value={profileForm.pool_range}
                                placeholder="ej. 192.168.20.2-192.168.20.200"
                            />
                            <span class="text-xs text-base-content/50 mt-1"
                                >Se creará pool-residencial-10m con este rango</span
                            >
                        </label>
                    {:else}
                        <label class="form-control">
                            <span class="label-text text-xs"
                                >Nombre del Pool Existente *</span
                            >
                            <input
                                class="input input-bordered input-sm"
                                type="text"
                                bind:value={profileForm.remote_address}
                                placeholder="ej. pool-clientes"
                            />
                        </label>
                    {/if}
                </div>
                ...
                <label class="form-control">
                    ...
                    <input
                        class="input input-bordered input-sm"
                        type="text"
                        bind:value={profileForm.comment}
...
{#if deleteProfileTarget !== null}
    <dialog class="modal modal-open">
        <div class="modal-box max-w-sm">
            ...
            <p class="py-4 text-sm">
                Se eliminarán el perfil y su pool de IPs asociado para <span
                    class="font-mono font-bold">{deleteProfileTarget}</span
                >. Los clientes con este perfil dejarán de conectarse.
            </p>
            <div class="modal-action">
                <button
                    class="btn btn-sm btn-ghost"
