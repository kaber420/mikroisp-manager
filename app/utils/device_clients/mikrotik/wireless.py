...
def get_wireless_type(api: RouterOsApi) -> Optional[str]:
    """
...
def get_aggregate_interface_stats(api: RouterOsApi) -> Dict[str, int]:
    ...
    filtered_interfaces = [
        n for n in physical_interfaces 
    ...
    for iface_name in filtered_interfaces:
        try:
             res = api.get_resource("/interface").call("monitor-traffic", {"interface": iface_name, "once": ""})
             if res:
                 data = res[0]
