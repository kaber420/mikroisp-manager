...
def provision_router_api_ssl(
    ...
    try:
        ...
        service_resource.set(
            id=get_id(api_ssl_service_list[0]), certificate=cert_name, disabled="no"
        )

        return {
            "status": "success",
            "message": "Router aprovisionado con API-SSL y usuario.",
        }
        ...
        for sensor in health_info:
            ...
            # Lógica para Formato A (Plano - Otros routers)
                ...
                if "cpu-temperature" in sensor:
                    data["cpu-temperature"] = sensor["cpu-temperature"]
                if "cpu-temp" in sensor:
                    data["cpu-temperature"] = sensor["cpu-temp"]
...
def get_interfaces(api: RouterOsApi) -> List[Dict[str, Any]]:
    try:
        ...
        return [
            ...
            if iface.get("type") in types_to_get and iface.get("name") != "none"
        ]
    except Exception as e:
...
def set_interface_status(
    ...
    api.get_resource(path).call(
        "set", {"id": interface_id, "disabled": "yes" if disabled else "no"}
    )
...
def kill_zombie_sessions(api: RouterOsApi, username: str) -> int:
    """
    Elimina sesiones API antiguas del usuario, intentando preservar la actual
    borrando desde la más vieja a la más nueva.
    """
    try:
        active_resource = api.get_resource("/user/active")
        # Filtramos solo nuestras sesiones API
        sessions = [
            s
        ...
        # Borrar las bajas primero elimina a los zombies.
        # La última (la más alta) suele ser la nuestra.
        def get_id_val(s):
            try:
        ...
        count = 0
        # Intentamos borrar todas. Si llegamos a la nuestra, la conexión caerá,
        # pero los zombies ya habrán muerto.
        for session in sessions:
            try:
                active_resource.remove(id=get_id(session))
                count += 1
            except Exception:
                # Si falla (ej. ya se desconectó), continuamos
                pass
        return count
    except Exception as e:
