...
class SwitchMonitorScheduler:
    ...
    def __init__(self, poll_interval: float = 5.0):
        self._running = False
        self._subscribed_switches: dict[
            str, dict
        ] = {}  # host -> {ref_count, last_unsubscribe_time}
    ...
    async def subscribe(self, host: str, creds: dict) -> None:
        ...
        try:
            ...
            logger.info(
                f"[SwitchMonitorScheduler] Subscribed to {host} (ref_count={info['ref_count']})"
            )
    ...
    async def unsubscribe(self, host: str) -> None:
        ...
        if info["ref_count"] <= 0:
            ...
            logger.info(
                f"[SwitchMonitorScheduler] Marked {host} for cleanup in {self.UNSUBSCRIBE_TIMEOUT}s"
            )
        else:
            logger.debug(
                f"[SwitchMonitorScheduler] Unsubscribed from {host} (ref_count={info['ref_count']})"
            )
