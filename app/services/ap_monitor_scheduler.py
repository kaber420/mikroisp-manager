...
class APMonitorScheduler:
    ...
    def __init__(self):
        self._running = False
        self._subscribed_aps: Dict[str, dict] = {}  # host -> {ref_count, last_unsubscribe_time, interval, last_poll_time}
        self.UNSUBSCRIBE_TIMEOUT = UNSUBSCRIBE_TIMEOUT
        logger.info(f"[APMonitorScheduler] Inicializado (Tick: {TICK_INTERVAL}s, Default Interval: {DEFAULT_POLL_INTERVAL}s)")
    ...
    async def subscribe(self, host: str, creds: dict, interval: int = None) -> None:
        ...
        if was_zero:
            ...
            logger.info(f"[APMonitorScheduler] Resubscribed to {host} (interval={effective_interval}s) - pending cleanup cancelled")
        
        try:
            await ap_connector.subscribe(host, creds)
            logger.info(f"[APMonitorScheduler] Subscribed to {host} (ref_count={info['ref_count']}, interval={effective_interval}s)")
