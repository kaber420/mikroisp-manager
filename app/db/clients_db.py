    ...
    try:
        cursor = conn.execute(
            ...
            (
                ...
                client_data.get("service_status"),
                client_data.get("billing_day"),
        ...
        cursor = conn.execute(
            "SELECT c.*, 0 as cpe_count FROM clients c WHERE c.id = ?", (new_client_id,)
        )
...
def update_client(client_id: int, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    cursor = conn.execute(
        f"UPDATE clients SET {set_clause} WHERE id = ?", tuple(values)
    )
    conn.commit()
...
def delete_client(client_id: int) -> int:
    ...
    try:
        conn.execute(
            "UPDATE cpes SET client_id = NULL WHERE client_id = ?", (client_id,)
        )
...
def get_cpes_for_client(client_id: int) -> List[Dict[str, Any]]:
    ...
    cursor = conn.execute(
        "SELECT mac, hostname, ip_address FROM cpes WHERE client_id = ?", (client_id,)
    )
    rows = [dict(row) for row in cursor.fetchall()]
    conn.close()
    return rows
...
def get_services_for_client(client_id: int) -> List[Dict[str, Any]]:
    conn = get_db_connection()
    cursor = conn.execute(
        "SELECT * FROM client_services WHERE client_id = ? ORDER BY created_at DESC",
        (client_id,),
    )
    rows = [dict(row) for row in cursor.fetchall()]
    conn.close()
    ...
    try:
        ...
        cursor = conn.execute(
            ...
            (
                ...
                data.get("router_secret_id"),
                data.get("profile_name"),
                data["suspension_method"],
                data.get("plan_id"),
                data.get("ip_address"),  # Nuevos campos
            ),
        )
