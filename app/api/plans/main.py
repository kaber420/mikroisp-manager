...
class Plan(PlanBase):
    ...
    router_name: Optional[str] = (
        None  # <--- Agregado para que el frontend pueda mostrar el router
    )
...
@router.get("/plans/router/{router_host}", response_model=List[Plan])
def get_plans_by_router(
    router_host: str, current_user: User = Depends(get_current_active_user)
):
    return plans_db.get_plans_by_router(router_host)


@router.post("/plans", response_model=Plan)
def create_plan(
    plan: PlanCreate, current_user: User = Depends(get_current_active_user)
):
    try:
