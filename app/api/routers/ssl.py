    ...
    try:
        ...
        if request.install_ca:
            ...
            if ca_result.get("status") == "error":
                raise HTTPException(status_code=500, detail=f"CA install failed: {ca_result.get('message')}")
        ...
        if request.method == ProvisionMethod.ROUTER_SIDE:
            ...
            try:
                ...
                if not success:
                    raise HTTPException(status_code=500, detail=f"CSR signing failed: {signed_cert}")
            ...
            if not success:
                raise HTTPException(status_code=500, detail=f"Certificate generation failed: {cert_pem}")
