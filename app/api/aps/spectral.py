                ...
                if channel.recv_ready():
                    ...
                    while '\n' in buffer:
                        ...
                        if match:
                            ...
                            if parsed_count == 1:
                                logger.info(f"[SpectralScan] First data parsed: freq={data_point['freq']} MHz")
                else:
                    time.sleep(0.05)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if msg == "stop":
                    await websocket.send_json({"status": "stopped", "message": "Escaneo detenido por el usuario"})
                    break
            except asyncio.TimeoutError:
                pass
            ...
            if data_points:
                ...
                if no_data_ticks > 600:
                    await websocket.send_json({
                        "status": "completed",
                        "message": "Escaneo completado (sin más datos)"
                    })
                    break
