...
async def get_router_by_host(session: AsyncSession, host: str) -> Router | None:
    ...
    try:
        ...
        logging.error(f"Error en router_repository.get_router_by_host para {host}: {e}")
        return None
...
async def get_all_routers(session: AsyncSession) -> Sequence[Router]:
    ...
    try:
        ...
        logging.error(f"Error en router_repository.get_all_routers: {e}")
        return []
...
async def update_router_in_db(session: AsyncSession, host: str, updates: dict[str, Any]) -> int:
    ...
    try:
        ...
        logging.error(f"Error en router_repository.update_router_in_db para {host}: {e}")
        await session.rollback()
        return 0
...
async def delete_router_from_db(session: AsyncSession, host: str) -> int:
    ...
    try:
        ...
        logging.error(f"Error en router_repository.delete_router_from_db para {host}: {e}")
        await session.rollback()
        return 0
...
async def update_router_status(session: AsyncSession, host: str, status: str, data: dict[str, Any] | None = None):
    ...
    try:
        ...
        logging.error(f"Error en router_repository.update_router_status para {host}: {e}")
