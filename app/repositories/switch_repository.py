...
Follows the same patterns as router_repository.py.
"""
...
async def get_switch_by_host(session: AsyncSession, host: str) -> Switch | None:
    ...
    try:
        ...
        logger.error(f"Error en switch_repository.get_switch_by_host para {host}: {e}")
        return None
...
async def get_all_switches(session: AsyncSession) -> Sequence[Switch]:
    ...
    try:
        ...
        logger.error(f"Error en switch_repository.get_all_switches: {e}")
        return []
...
async def update_switch_in_db(session: AsyncSession, host: str, updates: dict[str, Any]) -> int:
    ...
    try:
        ...
        logger.error(f"Error en switch_repository.update_switch_in_db para {host}: {e}")
        await session.rollback()
        return 0
...
async def delete_switch_from_db(session: AsyncSession, host: str) -> int:
    ...
    try:
        ...
        logger.error(f"Error en switch_repository.delete_switch_from_db para {host}: {e}")
        await session.rollback()
        return 0
...
async def update_switch_status(
    ...
    try:
        ...
        logger.error(f"Error en switch_repository.update_switch_status para {host}: {e}")
