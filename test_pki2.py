...
def create_csr(common_name="localhost"):
...
def test_pki():
    print("Testing generate_full_cert_pair with localhost...")
    success, key_pem, cert_pem = PKIService.generate_full_cert_pair("localhost")
    if success:
        print("[OK] generate_full_cert_pair passed.")
    else:
        print(f"[FAIL] generate_full_cert_pair failed: {cert_pem}")
        
    print("Testing sign_router_csr with localhost...")
    csr_pem = create_csr("localhost")
